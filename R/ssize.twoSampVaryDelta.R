#' Sample Size Calculations for Two-Sample Microarray Experiments with Differing Mean Expressions 
...
#' 
#' @param deltaMean location (mean) parameter of normal distribution followed by each \emph{delta.g}.
#' @param deltaSE scale (standard deviation) parameter of normal distribution followed by each \emph{delta.g}.
...
#' 
#' @details Each \emph{delta.g} is assumed to follow a Normal distribution with mean \code{deltaMean}
...
#' 
#' @seealso \code{\link{ssize.twoSamp}}, \code{\link{ssize.twoSampVary}}, \code{\link{ssize.oneSamp}}, \code{\link{ssize.oneSampVary}}, \code{\link{ssize.F}}, \code{\link{ssize.Fvary}}
#' 
#' @examples
...
#' @export
#' 
ssize.twoSampVaryDelta <- function (deltaMean, deltaSE, sigma, fdr = 0.05, power = 0.8, 
  ...
  if (side == "two-sided") {
    TSVaryDelta <- function(c) {
      ...
      dif <- abs((2 * pt(q = -c, df = 2 * n - 2)/
                    (1 - pt(q = c/sqrt(deltaSE^2 / (sigma^2 * 2/n) +1), df = 2 * n - 2, ncp = deltaMean/sqrt(deltaSE^2 + sigma^2 * 2/n))
                     +  pt(q = -c/sqrt(deltaSE^2 / (sigma^2 * 2/n) +1), df = 2 * n - 2, ncp = deltaMean/sqrt(deltaSE^2 + sigma^2 * 2/n))) - r))
      return(dif)
    }
  }
  if (side == "upper") {
    TSVaryDelta <- function(c) {
      r <- a * (1 - p)/((1 - a) * p)
      dif <- abs((2 * pt(q = -c, df = 2 * n - 2)/
                    (1 - pt(q = c/sqrt(deltaSE^2 / (sigma^2 * 2/n) +1), df = 2 * n - 2, ncp = deltaMean/sqrt(deltaSE^2 + sigma^2 * 2/n))) - r))
      return(dif)
    }
  }
  if (side == "lower") {
  ...
  for (i in 1:length(pi0)) {
    ...
    for (n in 3:N) {
      ...
      if (abs(ci - up.start) >= 1) {
        if (side == "two-sided") {
          pwr.new <- (1 - pt(q = ci/sqrt(deltaSE^2 / (sigma^2 * 2/n) +1), df = 2 * n - 2, ncp = deltaMean/sqrt(deltaSE^2 + sigma^2 * 2/n))
                      +  pt(q = -ci/sqrt(deltaSE^2 / (sigma^2 * 2/n) +1), df = 2 * n - 2, ncp = deltaMean/sqrt(deltaSE^2 + sigma^2 * 2/n)))
        }
      }
      if (abs(ci - up.start) < 1) {
        pwr.new <- 0
      ...
      crit <- c(crit, ci)
      pwr2 <- c(pwr2, pwr.new)
      if (pwr2[(i - 1) * (N - 2) + n - 2] >= power & ssize[i, 
                                                           1] == 0) {
        ssize[i, ] <- c(p, n, pwr2[(i - 1) * (N - 2) + 
                                     n - 2])
      }
    }
  }
  ...
  for (i in 1:length(pi0)) {
    if (i == 1) {
      plot(3:N, pwrMatrix[, i + 1], col = i, xlim = c(0, 
                                                      N), ylim = c(0, 1), xlab = "", ylab = "", pch = 16)
  ...
  mtext(bquote(paste("Average power vs. sample size with fdr=", 
                     .(fdr), ",")), cex = cex.title, padj = -1.85)
  mtext(bquote(paste(Delta[g], "~N(", .(round(deltaMean, 4)), 
                     ",", .(round(deltaSE, 4)), ") and ", sigma[g], " = ", 
                     .(round(sigma, 4)))), cex = cex.title, 
        padj = -0.1)
  legend(x = N, y = 0, xjust = 1, yjust = 0, col = 1:i, pch = c(16, 
                                                                16, 16), lty = 1:length(pi0), legend = as.character(pi0), 
         bg = "white", title = expression(pi[0]), cex = cex.legend)
  pwrMatrix <- round(pwrMatrix, 7)
  colnames(pwrMatrix) <- c("n", as.character(pi0))
  critMatrix <- matrix(c(3:N, crit), ncol = length(pi0) + 1, 
                       byrow = FALSE)
